# 30-16之聚合(3)---MapReduce
前兩篇說明完`mongodb`所提供的第一種聚合工作`aggregate framework`，本篇文章將要說明`mongodb`所提供的第二種聚合工作，`MapReduce`。
...
這個應用事實上有不少做法，這裡我們的作法是先用`query`塞選出`2`與`3`，然後在來計算它們的總收入，最後在用`finalize `來加上`dollar`。
